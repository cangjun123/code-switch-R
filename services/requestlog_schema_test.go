...
func TestEnsureRequestLogTableMigratesLegacyColumns(t *testing.T) {
	...
	for _, column := range []string{"cache_create_tokens", "cache_read_tokens", "reasoning_tokens", "client_ip", "first_token_duration_sec", "error_message", "requested_model", "response_model", "relay_key_name"} {
		if !columns[column] {
