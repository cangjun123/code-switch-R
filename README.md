...
后端调用 `/api/usage/token/`（携带 Key）、`/api/status` 和 `/api/pricing`（匿名，不携带 Key）。额度通过站点 `quota_per_unit` 换算为 USD 计费额度，保留原始数值；换算参数不可用时显示原始额度单位，不使用固定换算值。无限 Key 不代表账户余额充足。本功能不查询账户余额或面板登录信息，不计算今日消费和用量趋势。
