...
供应商配置统一在首页的 **Gemini** 标签中管理。对存在工具调用兼容问题的上游，
打开供应商的 **编辑 → 协议与高级 → 工具调用兼容修复** 并保存；新建供应商时也可以设置。
此选项默认关闭，按供应商生效，包含两项处理：

- 请求侧把工具定义的 `parametersJsonSchema` 原样迁移到转换网关支持的 `parameters`，
  保留字段、必填项及嵌套约束，同时适用于流式和非流式请求。已有 `parameters` 的定义保持不变。
- 响应侧把相邻的工具名称和参数残片合并成一个调用。

旧版“修复流式工具调用残片”开关的配置会继续生效，已开启的供应商无需重新配置。
此兼容处理不会为模型生成的调用补造参数；上游仍需支持工具定义中的 schema 约束。
...
- 如果某个 Responses 上游要求顶层必须有 `instructions`，可以开启 `Responses instructions 兼容`。relay 会在缺少 `instructions` 时，优先从首个 `developer/system message` 自动补齐；如果没有可提取内容，则回退为默认值 `You are a helpful assistant.`。
- 如果某个 Responses 上游要求顶层必须显式传 `store=false`，可以开启 `Responses store=false 兼容`，relay 会强制把顶层 `store` 设为 `false`。
- 如果某个 Responses 上游会因为不支持某些顶层字段而返回 400，可以在 `Responses 丢弃字段` 中填写字段名，relay 会在转发前删除它们。常见值有 `max_output_tokens`、`temperature`、`safety_identifier`。
...
在供应商编辑弹窗的“基本配置 → 上游信息”中选择 `sub2api`，复用当前 API Key 查询余额、额度、用量和计费倍率。“查询服务地址”留空时使用 API 地址并移除末尾 `/v1` 或 `/v1beta`；独立域名或子路径部署可填写服务根地址。保存前可点击“测试获取”，部分接口不可用不影响保存。

供应商卡片显示剩余额度、今日实扣和倍率；“详情”提供周期额度、到期时间、今日及累计 Token、近 30 天每日趋势和模型统计。钱包余额属于账户，用量属于当前 Key，同一账户的多张卡片余额不能相加。费用采用上游返回值，不以当前倍率重算历史费用，不修改 CodeSwitch 的本地价格、预算或转发策略；本期未接入模型单价或面板登录。

页面可见时每 5 分钟刷新当前标签页，也可手动刷新。接口失败时保留上次成功数据并标注时间；密钥或地址变化后不会沿用旧数据。旧版本不支持倍率接口时仍可显示额度与用量。

### New API 上游信息

在供应商编辑弹窗的“上游信息”中选择 `New API`，使用当前 API Key 查询 Key 剩余额度、已用额度、是否无限、到期时间和模型限制。支持普通供应商、Gemini 和自定义工具，地址配置与“测试获取”方式同 sub2api。模型限制仅展示，不会修改本地白名单。

可选填写上游个人设置中的“账户访问令牌”，部分版本还需要账户用户 ID（`New-Api-User`）。配置后额外只读调用 `/api/user/self`，成功时在卡片和详情中展示账户剩余额度，同时保留 Key 额度。未填写、令牌失效、用户 ID 不匹配或接口不可用时继续显示 Key 额度；查询失败不沿用旧账户余额。无需提供用户名密码。账户余额属于访问令牌对应的账户，请确认与 API Key 属于同一账户。

账户访问令牌仅发送到配置的查询站点账户接口，不发送到模型请求或公开价格接口。配置文件按当前系统用户读写权限保存（Linux `0600`）；“测试获取”和详情返回的数据不包含令牌或账户个人资料。

后端调用 `/api/usage/token/`（携带 Key）、`/api/status` 和 `/api/pricing`（匿名，不携带 Key）。额度通过站点 `quota_per_unit` 换算为 USD 计费额度，保留原始数值；换算参数不可用时显示原始额度单位，不使用固定换算值。无限 Key 不代表账户余额充足。未配置账户访问令牌时不查询账户余额，不计算今日消费和用量趋势。

详情页按“模型 + 公开分组”显示参考价格，支持模型搜索、分组筛选和每页 20 条。Token 价格单位为 USD / 百万 tokens，按次价格单位为 USD / 请求。公开分组倍率不代表当前 Key 的专属倍率；复杂表达式、插件及特殊计费只标注“复杂计费”，不推算单价。未知价格显示 `—`，明确为零的价格显示“免费”。价格展示不会修改本地计费或路由。

额度与换算信息缓存 5 分钟，公开价格缓存 30 分钟。手动刷新会更新三项数据，并遵守上游 `Retry-After`。各接口独立显示失败或旧数据状态；价格需要登录或不可访问时，仍可查看额度并保存配置。换算使用旧参数时也会标记，切换平台、密钥或查询地址后不沿用旧缓存。
