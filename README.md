...
- 如果某个 Responses 上游不支持 `max_output_tokens`，可以开启 `Responses max_output_tokens 兼容`，relay 会移除顶层 `max_output_tokens`。
- 如果某个 Responses 上游不支持 `temperature`，可以开启 `Responses temperature 兼容`，relay 会移除顶层 `temperature`。
